...
async function generateCoverPage(requestId, res) {
  ...
  doc.roundedRect(doc.page.width - 160, 140, 110, 32, 6).fill(priorityBg);
  doc.fontSize(13).fillColor('#ffffff').font('Helvetica-Bold')
    .text(request.priority.toUpperCase(), doc.page.width - 160, 150, { width: 110, align: 'center' });
  ...
  if (request.notes) {
    y += 10;
    doc.fontSize(11).fillColor('#6b7280').font('Helvetica-Bold').text('NOTES', labelX, y);
    y += 20;
    doc.rect(labelX, y, doc.page.width - 100, 80).fill('#f9fafb').stroke('#e5e7eb');
    doc.fontSize(13).fillColor('#374151').font('Helvetica')
      .text(request.notes, labelX + 10, y + 10, { width: doc.page.width - 120 });
    y += 90;
  }
  ...
  doc.fontSize(10).fillColor('#9ca3af').font('Helvetica')
    .text(
      `Generated by EduFlow • ${new Date().toLocaleString('en-GB')} • Request #${requestId}`,
